...
ARG RELEASE_NAME="alpha11"
ARG SUBDIR="alpha11"

RUN wget https://downloads.tuxfamily.org/godotengine/${GODOT_VERSION}/${SUBDIR}/Godot_v${GODOT_VERSION}-${RELEASE_NAME}_linux.64.zip \
    ...
    && unzip Godot_v${GODOT_VERSION}-${RELEASE_NAME}_linux.64.zip \
    && mv Godot_v${GODOT_VERSION}-${RELEASE_NAME}_linux.64 /usr/local/bin/godot \
    && unzip Godot_v${GODOT_VERSION}-${RELEASE_NAME}_export_templates.tpz \
    && mv templates/* ~/.local/share/godot/templates/${GODOT_VERSION}.${RELEASE_NAME} \
    && rm -f Godot_v${GODOT_VERSION}-${RELEASE_NAME}_export_templates.tpz Godot_v${GODOT_VERSION}-${RELEASE_NAME}_linux.64.zip
